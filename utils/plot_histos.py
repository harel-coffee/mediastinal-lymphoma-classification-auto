  ...
  if (n_rows != 1) and (n_cols != 1):
    ...
    for i in range(n_rows):
      for j in range(n_cols):
        ...
        x_1 = df[df["lymphoma_type"]==1][vars[n]]   # label "MHL"
        x_2 = df[df["lymphoma_type"]==2][vars[n]]   # label "MGZ"
        x_3 = df[df["lymphoma_type"]==3][vars[n]]   # label "MDLCBL"
        ...
        if plot_style == "stacked":
          ...
          ax[i,j] . hist ( x_1, range = (r_min, r_max), bins = 20, alpha = 0.7, color = colors[0], label = labels[0] )
          ax[i,j] . hist ( x_2, range = (r_min, r_max), bins = 20, alpha = 0.7, color = colors[1], label = labels[1] )
          ax[i,j] . hist ( x_3, range = (r_min, r_max), bins = 20, alpha = 0.7, color = colors[2], label = labels[2] )
  ...
  ### Histos in 1-D array
    ...
    for n in range ( n_rows * n_cols ):
      ...
      if plot_style == "stacked":
        ...
        r_min = min ( x_1.min(), x_2.min(), x_3.min() )
        r_max = max ( x_1.max(), x_2.max(), x_3.max() )
        ax[n] . hist ( x_1, range = (r_min, r_max), bins = 20, alpha = 0.7, color = colors[0], label = labels[0] )
        ax[n] . hist ( x_2, range = (r_min, r_max), bins = 20, alpha = 0.7, color = colors[1], label = labels[1] )
        ax[n] . hist ( x_3, range = (r_min, r_max), bins = 20, alpha = 0.7, color = colors[2], label = labels[2] )
